    ...
    while current:
        ...
        for node_i in current: #first node is start_id node e.g. "SUP-001"
            ...
            for dependent in dep_nodes.get(node_i, []): #dependant SUP-002
                leftover = max(0, delay_time - nodes[dependent]["inventory_days"]) #leftover delay time from inventory days
                if leftover > results.get(dependent, -1):
                    results[dependent] = leftover
                    if leftover > 0:
